...
class TestPortfolioVariance:
    ...
    def test_tc19_portfolio_variance_computes_correctly(self, cov_matrix_5_diverse):
        """
...
class TestMVPStructure:
    ...
    def test_tc22_no_short_selling(self, mvp_result_5_diverse):
        ...
        assert weights.min() >= -1e-6, f"Weight {weights.min()} violates non-negativity"
        assert weights.max() <= 1.0 + 1e-6, f"Weight {weights.max()} exceeds 1.0"
...
class TestMVPCorrectness:
    ...
    def test_tc23b_slsqp_converged(self, mvp_result_5_diverse):
        ...
        """
        assert (
            mvp_result_5_diverse["success"] is True
        ), f"Optimizer failed: {mvp_result_5_diverse['message']}"

    def test_tc24_mvp_reproducible(self, sample_tickers_diverse, date_range_full):
        """
...
class TestMVPSharpeRatio:
    ...
    def test_tc24b_sharpe_matches_formula(self, mvp_result_5_diverse, risk_free_rate):
        """
